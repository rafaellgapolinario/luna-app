...
export default function LunaIntro({ userName, onDone, onFinish }: LunaIntroProps) {
  ...
  async function playVoice(name?: string) {
    try {
      ...
      const who = name ? (', ' + name) : ''
      const text = greeting + who + '! Bem-vindo ao LUNA.'
  ...
  if (videoError) {
    return (
      ...
      >
        ...
        <div style={{ width: 130, height: 130, borderRadius: '50%', overflow: 'hidden', zIndex: 1, border: '2px solid rgba(124,109,250,0.5)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', zIndex: 1 }}>
          {[0,1,2,3,4].map(i => (
            <div key={i} style={{ width: 4, height: 20, borderRadius: 4, background: '#a78bfa' }} />
          ))}
        </div>
      </div>
    )
