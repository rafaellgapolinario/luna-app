...
export default function LoginPage() {
  ...
  const { accessToken, lang, setAuth, showToast } = useStore(s => ({
    accessToken: s.accessToken, lang: s.lang,
    setAuth: s.setAuth, showToast: s.showToast,
  }))
  const [showIntro, setShowIntro] = useState(false)
  const [introName, setIntroName] = useState('')

  // So redireciona se ja logado E nao estiver mostrando intro
  useEffect(() => {
    if (accessToken && !showIntro) router.replace('/luna')
  }, [accessToken, router, showIntro])

  // Carrega Google script
  useEffect(() => {
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, marginBottom: 40 }}>
            <div style={{
              ...
              boxShadow: '0 0 30px rgba(124,109,250,0.5), 0 0 60px rgba(124,109,250,0.2)',
            }}>
              <img src="/luna-logo.png" alt="LUNA" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            <div>
              <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 32, letterSpacing: -0.5, textAlign: 'center', background: 'linear-gradient(135deg,#c084fc,#818cf8,#60a5fa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                LUNA
              </h1>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', textAlign: 'center', marginTop: 6 }}>
          ...
          <button
            ...
            style={{
              ...
              padding: '14px 24px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)',
          ...
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', textAlign: 'center', marginTop: 20, lineHeight: 1.6 }}>
